...
class DecisionCoreNode(Node):
    ...
    def __init__(self):
        ...
        # Publisher for swarm state (for logging)
        ...
        # Timer for publishing state (1 Hz)
        self.create_timer(1.0, self.publish_swarm_state)
    ...
    def publish_swarm_state(self):
        ...
        if self.current_state is None:
            # If no state received, create mock state
            msg = SwarmState()
        else:
        ...
        # If no agents in state, add mock agents
        if not msg.agents:
        ...
        self.get_logger().debug(f"Published swarm state for {len(msg.agents)} agents")
